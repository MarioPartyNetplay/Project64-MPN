...
settings::settings(const string& settings_path) : settings_path(settings_path) {
    ...
    GetPrivateProfileString(L"plugin", L"dll", L"Project64-Input.dll", plugin_dll, MAX_PATH, utf8_to_wstring(settings_path).c_str());
    this->plugin_dll = wstring_to_utf8(plugin_dll);

    wchar_t name[256];
    GetPrivateProfileString(L"user", L"name", L"MPN Player", name, 256, utf8_to_wstring(settings_path).c_str());
